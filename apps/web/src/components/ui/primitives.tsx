...
export function Card({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return <div className={cn("rounded-surface border border-line bg-surface shadow-card", className)} {...props} />;
}
  ...
  return (
    <div className={cn("flex flex-wrap items-start justify-between gap-3", className)}>
      <div className="min-w-0">
        <As className="text-[17px] font-semibold tracking-tight text-ink">{title}</As>
...
export function Badge({ tone = "neutral", className, children, icon }: { tone?: Tone; className?: string; children: React.ReactNode; icon?: React.ReactNode }) {
  return (
    <span className={cn("inline-flex items-center gap-1.5 whitespace-nowrap rounded-md px-2 py-0.5 text-[13px] font-medium", toneClasses[tone], className)}>
      {icon}
      {children}
    </span>
  );
...
export function Skeleton({ className }: { className?: string }) {
  return <span className={cn("block animate-pw-pulse rounded-md bg-neutral-soft", className)} aria-hidden />;
}
...
export function Alert({ tone = "info", title, children, className, action }: { tone?: Tone; title?: React.ReactNode; children?: React.ReactNode; className?: string; action?: React.ReactNode }) {
  ...
  return (
    <div
      ...
      className={cn(
        "flex gap-3 rounded-surface border px-4 py-3 text-sm",
...
export function EmptyState({ icon, title, description, action, className }: { icon?: React.ReactNode; title: string; description?: React.ReactNode; action?: React.ReactNode; className?: string }) {
  return (
    <div className={cn("flex flex-col items-center px-6 py-12 text-center", className)}>
      {icon ? <div className="mb-3 flex size-11 items-center justify-center rounded-full bg-primary-soft text-primary [&_svg]:size-5">{icon}</div> : null}
...
export function DefinitionRow({ label, children, className }: { label: string; children: React.ReactNode; className?: string }) {
  return (
    <div className={cn("grid grid-cols-1 gap-1 py-2 sm:grid-cols-[180px_1fr] sm:gap-4", className)}>
...
export function StepNumber({ n, tone = "primary", className }: { n: number | string; tone?: "primary" | "muted" | "done"; className?: string }) {
  return (
    <span
      className={cn(
        "flex size-8 shrink-0 items-center justify-center rounded-full text-sm font-semibold tabular",
        tone === "primary" && "bg-primary-soft text-primary-ink",
        tone === "muted" && "bg-neutral-soft text-neutral-ink",
