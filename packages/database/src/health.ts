...
export async function getOperationsHealth(sql: Sql, ctx: ServiceContext) {
  ...
  const workerStatus = completedAgo == null ? "NEVER_SEEN" : completedAgo > 180 ? "STALE" : "HEALTHY";
  return {
    worker: {
      status: workerStatus,
      ...
      guidance:
        workerStatus === "HEALTHY"
          ? null
          : "Background checking is delayed. Start the worker with `npm run dev:worker` and confirm DATABASE_URL. Queued work remains visible and will resume.",
    },
